...
function [time,obj,Vsol,Ksol] = SolveCovarianceSteering(PS, dynamicsSelectFlag)   
    %%% Lower Stage %%%
    N = PS.N;
    nx = PS.nx;
    nu = PS.nu;
    ...
    % Chance Constraints
    Constraints = chanceConstraints(Constraints,EX,S,IplusBK,V,K,PS, dynamicsSelectFlag);
...
function Constraints = chanceConstraints(Constraints,EX,S,IplusBK,V,K,PS, dynamicsSelectFlag)
    a_x = PS.alphax;
    a_u = PS.alphau;
    b_x = PS.betax;
    b_u = PS.betau;
