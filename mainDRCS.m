...
hold on; 
grid on;
plot(PS.dt*[1:N],PS.deltax(2,:),'*k','MarkerSize',30);
plot(PS.dt*[1:N],PS.deltax(3,:),'or','MarkerSize',20,'LineWidth',1);
ylim([0 9E-04]);
...
hold on; 
grid on;
plot(PS.dt*[1:N],deltabar(2,:),'*k','MarkerSize',30);
plot(PS.dt*[1:N],deltabar(3,:),'or','MarkerSize',20,'LineWidth',1);
ylim([0 9E-04]);
...
if (dynamicsSelectFlag == 1)
    figNum = figNum + 1;
    PostProcess(figNum,numMonteTrials,Vsol,Ksol,PS);
%     figNum = figNum + 1;
%     PostProcess3D(figNum,numMonteTrials,Vsol,Ksol,PS,1);
%     V_N = computeVolumeN(PS,Ksol);
elseif(dynamicsSelectFlag == 2)
    figNum = figNum + 1;
    PostProcessDoubleIntegrator(figNum,numMonteTrials,Vsol,Ksol,PS);
end